...
const (
	...
	envLoggingFile = "ZAP_FILE" // /path/to/file
	envLoggingURL  = "ZAP_URL"  // url that will be processed by sink in the zap

	envLoggingOutput = "ZAP_OUTPUT"     // possible values: stdout|stderr|file combine multiple values with '+'
	envLoggingLabels = "ZAP_LOG_LABELS" // comma-separated key-value pairs, i.e. "app=example_app,dc=sjc-1"
)
...
var ISO8601TimeFormat = "2006-01-02T15:04:05.000Z0700" // go-log use as default

type Config struct {
	// Format overrides the time format in the log output. Defaults to time.Kitchen
	TimeFormat string

	// LogFormat overrides the format of the log output. Defaults to ColorizedOutput
	LogFormat LogFormat
...
func SetupLogging(cfg Config) {
	...
	if len(cfg.File) > 0 {
		if path, err := filepath.Abs(cfg.File); err != nil {
...
func configFromEnv() Config {
	...
	switch format {
	case "color":
		cfg.LogFormat = ColorizedOutput
	case "nocolor":
		cfg.LogFormat = PlaintextOutput
	case "json":
		cfg.LogFormat = JSONOutput
	default:
		if format != "" {
